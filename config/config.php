...
$base_url = 'https://localhost/'; // Thay url web bạn
...
class PNH
{
    ...
    function connect()
    {
        if (!$this->ketnoi)
        {
            $this->ketnoi = mysqli_connect('localhost', 'root', '', 'eximbank') or die ('Vui lòng kết nối đến DATABASE'); 
...
function CheckLogin()
{
    ...
    if($my_username != True)
    {
        return die('<script type="text/javascript">setTimeout(function(){ location.href = "'.BASE_URL('Auth').'" }, 0);</script>');
    }
}
function CheckAdmin()
{
