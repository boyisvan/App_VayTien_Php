...
?>
<?php
if(isset($_GET['id']) && ($getUser['level'] == 'admin' || $getUser['level'] == 'sup'))
{
    $row = $PNH->get_row(" SELECT * FROM `users` WHERE `id` = '".check_string($_GET['id'])."'  ");
    if(!$row)
    {
        admin_msg_error("Người dùng này không tồn tại", BASE_URL(''), 2000);
    }
}
else
{
    admin_msg_error("Liên kết không tồn tại", BASE_URL(''), 2000);
}
if(isset($_POST['btnCongTien']) && isset($_POST['value']) && isset($row['username']) && ($getUser['level'] == 'admin' || $getUser['level'] == 'sup'))
{
    $value = check_string($_POST['value']);
    $ghichu = check_string($_POST['ghichu']);
    $km = check_string($_POST['km']);
    ...
    $create = $PNH->insert("dongtien", [
        ...
        'sotienthaydoi' => $value + ($value * $km / 100),
        'sotiensau' => $row['money'] + $value + ($value * $km / 100),
        'thoigian' => gettime(),
        'noidung' => $getUser['level'].' cộng tiền ('.$ghichu.')',
        'username' => $row['username']
    ]);
    if($create)
    {
        $PNH->cong("users", "money", $value + ($value * $km / 100), " `username` = '".$row['username']."' ");
...
if(isset($_POST['btnTruTien']) && isset($_POST['value']) && isset($row['username']) && ($getUser['level'] == 'admin' || $getUser['level'] == 'sup'))
{
    $value = check_string($_POST['value']);
    $ghichu = check_string($_POST['ghichu']);
    $km = check_string($_POST['km']);
    ...
    $create = $PNH->insert("dongtien", [
        ...
        'sotiensau' => $row['money'] - ($value + $value * $km / 100),
        'thoigian' => gettime(),
        'noidung' => $getUser['level'].' trừ tiền ('.$ghichu.')',
        'username' => $row['username']
    ]);
    if($create)
    {
        $PNH->tru("users", "money", ($value + $value * $km / 100), " `username` = '".$row['username']."' ");
        $PNH->tru("users", "total_money", $value, " `username` = '".$row['username']."' ");
                    ...
                    <div class="card-body">
                        <form action="" method="POST">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-8">
                                    <div class="form-line">
                                        <input type="text" class="form-control" name="value" id="stc" required>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="inputEmail3" class="col-sm-4 col-form-label">Phần trăm KM (%)</label>
                                <div class="col-sm-8">
                                    <div class="form-line">
                    ...
                    <div class="card-body">
                        <form action="" method="POST">
                            <div class="form-group row">
                                ...
                                <div class="col-sm-8">
                                    <div class="form-line">
                                        <input type="text" class="form-control" name="value" id="sttru" required>
                                    </div>
                                </div>
                            </div>
                           <div class="form-group row">
                        ...
                        <div class="table-responsive">
                            <table id="datatable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 1;
                                    foreach($PNH->get_list(" SELECT * FROM `dongtien` WHERE `username` = '".$row['username']."' ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td><?=$i++;?></td>
